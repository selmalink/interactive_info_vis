(function () {
  window.registerSketch('hw5', function (p) {
    let W = 1080, H = 900;
    const m = { top: 180, right: 28, bottom: 72, left: 72 }; 
    const legendW = 280;
    ...
    p.mousePressed = () => {
      for (const b of UI.buttons) {
        if (hitRect(p.mouseX, p.mouseY, b)) { b.onClick(); p.redraw(); return; }
      }
    };
    p.mouseMoved = () => { p.redraw(); };
    p.touchMoved = () => { p.redraw(); return false; };
    p.mouseOut = () => { p.cursor(p.ARROW); };
    ...
    function drawCenteredTitle() {
      const title    = 'The Eye-Level Trap: How Shelf Height Hides the Sugary Truth';
      const subtitle = 'Cereal Shelf Placement vs. Nutrition Quality';
      const info     = 'This visualization explores how grocery shelf placement relates to cereal nutrition. Each circle represents a cereal, sized by calories and colored by shelf height. Notice how eye-level shelves (green and pink) tend to feature higher-sugar, lower-fiber cereals — the ones most visible to shoppers. Use the controls to highlight eye-level shelves and focus on less healthy cereals.';
      ...
      p.fill('#334155');
      const subY = 28 + 34 + 10;
      p.text(subtitle, W / 2, subY);

      
      p.textSize(14);
      p.fill('#55677f');
      const infoTop = subY + 26;
      const infoBottom = textWrapCentered(info, W / 2, infoTop, W - legendW - 120, 18);
      legendTop = infoBottom + 10; 
    }
    ...
    function drawQuadrantHighlights() {
      const vx = xScale(midVal), hy = yScale(midVal);
      p.noStroke();
      p.fill(34, 197, 94, 26); // top-left: Low sugar • High fiber
      p.rect(plot.x0, plot.y0, vx - plot.x0, hy - plot.y0);
      p.fill(239, 68, 68, 26); // bottom-right: High sugar • Low fiber
    ...
    function drawLegendAndControls() {
      ...
      const lx = W - m.right - legendW,
            lw = legendW,
            pad = 14,
            ly = Math.max(m.top, legendTop); 

      p.stroke('#e5e7eb'); p.fill(255);
      p.rect(lx, ly, lw, 290, 12);
      ...
      p.fill('#111827'); p.textSize(12);
      p.text('Dot size = calories', lx + pad, yy + 6);

      yy += 38; p.fill('#55677f'); p.textSize(12); p.text('Try these buttons:', lx + pad, yy);

      const btnW = lw - pad * 2 - 4;
      const b1 = drawButton(lx + pad + 2, yy + 12, btnW, 30,
      ...
      const b2 = drawButton(lx + pad + 2, yy + 52, btnW, 30,
    ...
    function drawTooltip() {
      ...
      for (const d of rows) {
        ...
        const rHit = Math.max(12, d._r * 0.6);
    ...
    function textWrapCentered(str, cx, y, maxWidth, lineH) {
      ...
      if (line) { p.text(line, cx, yy); yy += lineH; }
      return yy;
    }
  });
})();
