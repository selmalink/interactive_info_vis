// sketches/HW5.js — HW5 narrative viz: Sugar vs Fiber by Shelf (quadrant highlights, no evidence panel)
(function () {
  window.registerSketch('hw5', function (p) {
    ...
    const shelfColors = {
      1: p => p.color(33, 113, 181),   // blue
      2: p => p.color(51, 160, 44),    // green
      3: p => p.color(251, 154, 153)   // pink
    };
    ...
    function layout() {
      ...
      const side = Math.max(320, Math.min(availW, availH));
      plot.w = plot.h = side;
    ...
    function drawTitle() {
      ...
      p.textSize(18);
      p.text('Eye-Level Advantage? — Cereal Shelf Height vs Sugar • Fiber', m.left, 6);
      p.textSize(12); p.fill('#6b7280');
      p.text('Each dot is a cereal — size: calories, color: shelf height (1=low, 3=high). Hover for details.', m.left, 26);
    }

    function drawAxes() {
      p.stroke('#0f172a'); p.strokeWeight(1.5); p.noFill();
    ...
    function drawQuadrantHighlights() {
      ...
      p.noStroke();
      p.fill(34, 197, 94, 28); // soft green
      p.rect(plot.x0, plot.y0, vx - plot.x0, hy - plot.y0);

      p.fill(239, 68, 68, 28); // soft red
    ...
    function drawLegendAndUI() {
      UI.buttons.length = 0;

      const lx = W - m.right - legendW, ly = m.top, lw = legendW, pad = 14;
      const cardH = 220;
      p.stroke('#e5e7eb'); p.fill(255); p.rect(lx, ly, lw, cardH, 12);
      ...
      yy += 18;
      const b1 = drawButton(lx + pad, yy + 16, lw - pad * 2, 30,
      ...
      const b2 = drawButton(lx + pad, yy + 56, lw - pad * 2, 30,
    ...
    function jitter(key) {
      ...
      const jx = ((h % 7) - 3) * 0.6, jy = (((h >> 3) % 7) - 3) * 0.6;
      return { x: jx, y: jy };
    }
